...
async function openManualCockpit(user: ReturnType<typeof userEvent.setup>) {
  ...
  await user.click(screen.getByRole('button', { name: /Weiter/i }))
  await user.click(screen.getByRole('button', { name: /Fallbasis vorbereiten/i }))
...
describe('Kodierpfad prototype', () => {
  ...
  it('führt vom Fallkontext bis zum Cockpit', async () => {
    ...
    expect(screen.getByRole('navigation', { name: /Arbeitsweg vom KIS zum Fallabschluss/i })).toHaveTextContent('KIS-Start')
    expect(screen.getByText(/Profil aktiv/i)).toBeInTheDocument()

    await user.click(screen.getByRole('button', { name: /Weiter/i }))
    expect(screen.getByRole('heading', { name: /Ausgangsunterlagen/i })).toBeInTheDocument()

    await user.click(screen.getByRole('button', { name: /Weiter/i }))
    expect(screen.getByRole('heading', { name: /Bereit, die Fallbasis zu prüfen/i })).toBeInTheDocument()
    ...
    expect(screen.getByText(/Ausgangskodierung aus dem KIS/i)).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: /Fallbasis bestätigen/i }))
    ...
    expect(caseMap).toHaveTextContent('DRG-Stabilität')
    expect(caseMap).toHaveTextContent('Belegsicherheit')
    expect(caseMap).toHaveTextContent('Kodiersicherheit')
    expect(within(caseMap).getByLabelText(/Reihenfolge der offenen Prüfungen/i)).toHaveTextContent('Jetzt klären')
    ...
    expect(screen.getByRole('button', { name: /KIS-Ausgang/i })).toBeInTheDocument()
