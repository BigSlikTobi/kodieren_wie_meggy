...
describe('Kodierpfad prototype', () => {
  ...
  it('führt vom Fallkontext bis zum Cockpit', async () => {
    ...
    expect(documentAssessment).toHaveTextContent('Vorkodierung')
    expect(documentAssessment).toHaveTextContent('Jetzt klären')
    expect(documentAssessment).toHaveTextContent('Validiert')
    expect(documentAssessment).toHaveTextContent('Vorläufig stimmig')
    expect(documentAssessment).toHaveTextContent('Vermutlich nicht relevant')
    expect(documentAssessment).toHaveTextContent('Bronchoskopie- und Biopsieberichtvorhanden · Nachvalidierung nötig · 1 Vorkode')
    expect(documentAssessment).toHaveTextContent('Medikations- und Therapienachweisfehlend · potentiell relevant')
    await user.click(screen.getByRole('button', { name: /Fallbasis bestätigen/i }))
